...
def build_balanced(initial_input: list[int | str], parent: Node | None) -> Optional[Node]:

    if len(initial_input) == 0:
        return None

    mid_index = len(initial_input) // 2
    node = Node(value=initial_input[mid_index], left=None, right=None, parent=parent)
